CLASS ycl_aai_rest_prog_include_mcp DEFINITION INHERITING FROM ycl_aai_rest_base
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_request_create_s,
             name              TYPE yde_aai_fc_include_name,
...
CLASS ycl_aai_rest_prog_include_mcp IMPLEMENTATION.
  ...
  METHOD yif_aai_rest_resource~create.
    ...
    DATA(l_response) = NEW ycl_aai_fc_include_tools( )->create( i_include_name      = ls_request-name
  ...
  METHOD yif_aai_rest_resource~read.
    ...
    IF l_action IS INITIAL.
      ...
      IF l_package IS INITIAL.
        ...
        l_response = NEW ycl_aai_fc_include_tools( )->read( i_include_name = CONV #( l_name ) ).

      ELSE.

        l_response = NEW ycl_aai_fc_include_tools( )->search( i_package           = CONV #( l_package )
                                                              i_include_name      = CONV #( l_name )
    ...
    CASE l_action.
      ...
      WHEN 'CHECK'.
        ...
        l_response = NEW ycl_aai_fc_include_tools( )->check_syntax( i_include_name = CONV #( l_name ) ).

      WHEN 'ACTIVATE'.

        l_response = NEW ycl_aai_fc_include_tools( )->activate( i_include_name = CONV #( l_name ) ).
  ...
  METHOD yif_aai_rest_resource~update.
    ...
    DATA(l_response) = NEW ycl_aai_fc_include_tools( )->update( i_include_name      = ls_request-name
                                                                i_short_description = ls_request-short_description
                                                                i_transport_request = ls_request-transport_request
