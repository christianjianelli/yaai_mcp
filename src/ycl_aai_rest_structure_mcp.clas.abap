CLASS ycl_aai_rest_structure_mcp DEFINITION INHERITING FROM ycl_aai_rest_base
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_request_create_s,
             ...
             components        TYPE ytt_aai_fc_struct_fields,
...
CLASS ycl_aai_rest_structure_mcp IMPLEMENTATION.
  ...
  METHOD yif_aai_rest_resource~create.
    ...
    l_response = NEW ycl_aai_fc_structure_tools( )->create( i_structure_name    = ls_request-name
  ...
  METHOD yif_aai_rest_resource~read.
    ...
    IF l_package IS INITIAL.
      ...
      l_response = NEW ycl_aai_fc_structure_tools( )->read( i_structure_name = CONV #( l_name ) ).

    ELSE.

      l_response = NEW ycl_aai_fc_structure_tools( )->search( i_package           = CONV #( l_package )
                                                              i_structure_name    = CONV #( l_name )
  ...
  METHOD yif_aai_rest_resource~update.
    ...
    l_response = NEW ycl_aai_fc_structure_tools( )->update( i_structure_name    = ls_request-name
                                                            i_short_description = ls_request-short_description
                                                            i_transport_request = ls_request-transport_request
                                                            i_t_components      = ls_request-components ).
  ...
  METHOD yif_aai_rest_resource~delete.
    ...
    DATA(l_response) = NEW ycl_aai_fc_structure_tools( )->delete( i_structure_name = CONV #( l_name )
