CLASS ycl_aai_rest_oo_interface_mcp DEFINITION INHERITING FROM ycl_aai_rest_base
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_interface_create_s,
           ...
           END OF ty_interface_create_s,

           BEGIN OF ty_interface_update_s,
             name              TYPE yde_aai_class_name,
             short_description TYPE as4text,
             transport_request TYPE yde_aai_fc_transport_request,
             source_code       TYPE string,
           END OF ty_interface_update_s.
...
CLASS ycl_aai_rest_oo_interface_mcp IMPLEMENTATION.

  METHOD yif_aai_rest_resource~create.

    DATA ls_request TYPE ty_interface_create_s.
    ...
    DATA(l_response) = NEW ycl_aai_fc_oo_interface_tools( )->create( i_interface_name    = ls_request-name
  ...
  METHOD yif_aai_rest_resource~read.
    ...
    IF l_action IS INITIAL.
      ...
      IF l_package IS INITIAL.
        ...
        l_response = NEW ycl_aai_fc_oo_interface_tools( )->read( i_interface_name = CONV #( l_name ) ).

      ELSE.

        l_response = NEW ycl_aai_fc_oo_interface_tools( )->search( i_package           = CONV #( l_package )
                                                                   i_interface_name    = CONV #( l_name )
    ...
    CASE l_action.
      ...
      WHEN 'CHECK'.
        ...
        l_response = NEW ycl_aai_fc_oo_interface_tools( )->check_syntax( i_interface_name = CONV #( l_name ) ).

      WHEN 'ACTIVATE'.

        l_response = NEW ycl_aai_fc_oo_interface_tools( )->activate( i_interface_name = CONV #( l_name ) ).
  ...
  METHOD yif_aai_rest_resource~update.
    ...
    DATA ls_request TYPE ty_interface_update_s.
    ...
    DATA(l_response) = NEW ycl_aai_fc_oo_interface_tools( )->update( i_interface_name    = ls_request-name
                                                                     i_short_description = ls_request-short_description
                                                                     i_transport_request = ls_request-transport_request
